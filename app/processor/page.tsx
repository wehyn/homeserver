...
export default function ProcessorPage() {
  ...
  return <main className="shell memory-shell">
    ...
    <section className="content memory-content">
      <header className="topbar"><div className="breadcrumb"><Link href="/">Workspace</Link><span>/</span><strong>Processor</strong></div><div className="top-actions"><span className="last-sync"><span className="sync-dot" />{snapshot ? `Updated ${formatTime(snapshot.updatedAt)}` : "Loading processor"}</span><button className="icon-button" onClick={() => void refresh()} title="Refresh processor details" aria-label="Refresh processor details"><RefreshCw size={17} className={refreshing ? "spin" : ""} /></button><button className="avatar-button">D</button></div></header>
      <div className="main-inner memory-inner">
        <div className="memory-page-heading"><div><Link className="back-link" href="/"><ArrowLeft size={14} />Overview</Link><p className="eyebrow">System detail</p><h1>Processor</h1><p className="subheading">Host processes using CPU on this device.</p></div><div className="memory-refresh-note"><span className="sync-dot" />Live · 5 sec</div></div>
