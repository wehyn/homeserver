...
export function ActivityRow({ activity, now }: { activity: ActivityEvent; now?: number }) {
  return <div className="activity-row"><span className={`activity-icon ${activityTone(activity)}`}>{activityIcon(activity)}</span><div><strong>{activityTitle(activity)}</strong><small>{formatRelativeTime(activity.createdAt, now)}</small></div><ChevronDown size={14} className="activity-arrow" /></div>;
}
